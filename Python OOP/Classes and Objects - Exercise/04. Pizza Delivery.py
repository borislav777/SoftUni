...
class PizzaDelivery:
    ...
    def remove_ingredient(self, ingredient: str, quantity: int, price_per_ingredient: float):
        if not self.ordered:
            ...
            elif ingredient in self.ingredients and quantity > self.ingredients.get(ingredient):
    ...
    def make_order(self):
        ...
        return f"You've ordered pizza {self.name} prepared with " \
               f"{', '.join([f'{key}: {value}' for key, value in self.ingredients.items()])} and the price will be " \
               f"{self.price}lv."
